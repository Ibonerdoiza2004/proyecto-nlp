...
def intervals_overlap(a_start, a_end, b_start, b_end) -> float:
    """Devuelve duración del solape en segundos (o 0 si no hay)."""
    start = max(a_start, b_start)
    end = min(a_end, b_end)
    return max(0.0, end - start)

def best_speaker_for_segment(seg, spk_segments):
    """Dado un segmento de texto (start/end), devuelve la etiqueta de speaker con mayor solape."""
...
def write_rttm(spk_segments, path: Path, uri: str):
    """
    Formato:
    SPEAKER <uri> 1 <start> <dur> <ortho> <stype> <name> <conf>
    """
...
def main():
    ...
    # === TUS RUTAS ===
    in_dir = Path("./audios/pizarra")  # separados pizarra y media inglesa
    out_diar = Path("./diarizado_v3/pizarra")  # carpeta de salida
    out_diar.mkdir(parents=True, exist_ok=True)
    n_hablantes = 4  # ajustar según caso

    transcripts_dir = Path("./transcripts/pizarra")  # separados pizarra y media inglesa
    ...
    # === TOKEN HF ===
    # 1) Intenta leerlo de la variable de entorno
    ...
    for ap in audio_paths:
        ...
        if (out_diar / f"{stem}.diarized.json").exists():
            ...
            continue
        # v4: sigue aceptando num_speakers (también min/max_speakers)
        try:
            # 1) Cargar MP3 en memoria y pasar al pipeline
            waveform, sr = torchaudio.load(str(ap))  # ffmpeg/sox-backend via torchaudio
            result = pipeline({"waveform": waveform, "sample_rate": sr},
                            num_speakers=int(n_hablantes))
        except Exception as e:
            # 2) Fallback: si la decodificación a chunks falla, reconvertimos a WAV 16k mono y reintentamos
        ...
        # v4: el resultado suele exponer .speaker_diarization (Annotation).
        # En algunos setups podría devolver directamente una Annotation.
        annotation = getattr(result, "speaker_diarization", result)

        # Convertir a lista simple de segmentos de speaker
        spk_segments = []
        try:
            # API estándar de Annotation (pyannote.core) – igual que en v3
            for turn, _, speaker in annotation.itertracks(yield_label=True):
                # Normaliza etiqueta a SPEAKER_00 si viene como int
            ...
            # Fallback por si el iterador devuelve (segment, speaker) directamente (algunos ejemplos v4)
            try:
        ...
        # Fusionar con transcripción (si existe)
