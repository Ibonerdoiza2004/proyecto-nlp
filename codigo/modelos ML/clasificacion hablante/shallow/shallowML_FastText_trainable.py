"""
Shallow ML con FastText Trainable
FastText con embeddings entrenables (char n-grams) + 6 clasificadores tradicionales
"""
...
print("="*60)
print("SHALLOW ML + FASTTEXT TRAINABLE")
print("="*60)
...
# Entrenar FastText trainable con char n-grams
print("\nEntrenando FastText trainable (char n-grams 3-6)...")
ft_model = FastText(sentences=X_train, vector_size=100, window=5, min_count=2, workers=4, 
                    sg=1, epochs=15, min_n=3, max_n=6)

def get_avg_fasttext(text, model):
    """Promedio de embeddings FastText para clasificadores shallow"""
    vectors = [model.wv[word] for word in text if word in model.wv]
    return np.mean(vectors, axis=0) if vectors else np.zeros(model.vector_size)

X_train_ft = np.array([get_avg_fasttext(text, ft_model) for text in X_train])
X_test_ft = np.array([get_avg_fasttext(text, ft_model) for text in X_test])
...
for name, config in classifiers.items():
    ...
    y_pred = best_model.predict(X_test_ft)
...
plt.title(f'Shallow ML + FastText Trainable - {best_classifier[0]}')
plt.ylabel('True Label')
plt.xlabel('Predicted Label')
plt.tight_layout()
plt.savefig('confusion_matrix_shallowML_fasttext_trainable.png', dpi=300)

# Comparación
plt.figure(figsize=(12, 6))
models = list(results.keys())
accuracies = [results[m]['accuracy'] for m in models]
plt.bar(models, accuracies, color='lightcoral', edgecolor='darkred')
plt.ylabel('Accuracy')
plt.title('Shallow ML + FastText Trainable - Comparación')
...
plt.tight_layout()
plt.savefig('comparison_shallowML_fasttext_trainable.png', dpi=300)
print("\n✓ Completado")
