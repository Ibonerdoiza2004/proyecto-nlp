...
print("SHALLOW ML + BETO MEAN POOLING")
...
train_pos, test_pos, y_train, y_test = train_test_split(pos_indices, y_encoded, test_size=0.2, random_state=10, stratify=y_encoded)

# Cargar embeddings de BETO mean pooling
...
classifiers = {
    'Logistic Regression': LogisticRegression(max_iter=2000, random_state=10),
...
plt.title(f'Matriz de Confusión - {best_model_name} + BETO')
plt.ylabel('Real')
...
plt.tight_layout()
plt.savefig('comparison_shallowML_bert.png', dpi=300, bbox_inches='tight')