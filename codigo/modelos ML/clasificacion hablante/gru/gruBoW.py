"""
GRU con Bag of Words (BoW) Embeddings
BoW → Dense embedding layer → GRU Bidireccional → Dense
"""
...
MAX_FEATURES, EMBEDDING_DIM, HIDDEN_DIM, NUM_LAYERS = 5000, 128, 128, 2
...
model = GRUBoWClassifier(X_train_bow.shape[1], EMBEDDING_DIM, HIDDEN_DIM, NUM_LAYERS, num_classes, DROPOUT).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_gru_bow.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_gru_bow.pth'))
model.eval()
...
plt.title('GRU + BoW')
plt.tight_layout()
plt.savefig('confusion_matrix_gru_bow.png', dpi=300)
print("✓ Completado")
