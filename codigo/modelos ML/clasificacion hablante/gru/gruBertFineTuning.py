"""
GRU con BERT Fine-tuning Completo
Fine-tuning end-to-end de BERT + GRU para clasificación
"""
...
BERT_LR, GRU_LR = 2e-5, 1e-4

print("GRU + BERT FINE-TUNING")
...
class GRUBertFineTuning(nn.Module):
    def __init__(self, bert_model_name, hidden_dim, num_layers, num_classes, dropout):
        super(GRUBertFineTuning, self).__init__()
        self.bert = AutoModel.from_pretrained(bert_model_name)
        bert_dim = self.bert.config.hidden_size
        self.gru = nn.GRU(bert_dim, hidden_dim, num_layers=num_layers, batch_first=True,
...
model = GRUBertFineTuning(BERT_MODEL, HIDDEN_DIM, NUM_LAYERS, num_classes, DROPOUT).to(device)

# Differential learning rates
bert_params = list(model.bert.parameters())
gru_params = list(model.gru.parameters()) + list(model.fc.parameters())
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_gru_bert_finetuning.pth')

model.load_state_dict(torch.load('models/best_gru_bert_finetuning.pth'))
model.eval()
...
plt.title('GRU + BERT Fine-tuning')
plt.tight_layout()
plt.savefig('confusion_matrix_gru_bert_finetuning.png', dpi=300)
print("✓ Completado")
