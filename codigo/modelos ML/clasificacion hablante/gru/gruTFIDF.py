"""
Clasificación de Hablantes usando GRU con TF-IDF
Arquitectura: TF-IDF → Dense (embedding denso) → GRU Bidireccional → Dense
Técnicas: TF-IDF vectorization, Dense embedding layer, Bidirectional GRU, Gradient Clipping
Fuentes: PDF págs 15-18 (TF-IDF), págs 38-40 (GRU bidireccional)
"""
...
print("="*60)
print("GRU BIDIRECCIONAL + TF-IDF")
print("="*60)
...
# Modelo GRU con TF-IDF
class GRUTFIDFClassifier(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, num_layers, num_classes, dropout):
        super(GRUTFIDFClassifier, self).__init__()
...
# Crear modelo
print("\n" + "="*60)
print("CONSTRUYENDO MODELO GRU + TF-IDF")
print("="*60)

model = GRUTFIDFClassifier(
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_gru_tfidf.pth')
        print(f"✓ Mejor modelo guardado (val_acc: {val_acc:.4f})")

# Cargar mejor modelo
model.load_state_dict(torch.load('models/best_gru_tfidf.pth'))
...
plt.title('Matriz de Confusión - GRU + TF-IDF')
plt.ylabel('Real')
plt.xlabel('Predicción')
plt.tight_layout()
plt.savefig('confusion_matrix_gru_tfidf.png', dpi=300, bbox_inches='tight')
...
axes[0].set_title('Accuracy - GRU + TF-IDF')
...
plt.tight_layout()
plt.savefig('training_history_gru_tfidf.png', dpi=300, bbox_inches='tight')
