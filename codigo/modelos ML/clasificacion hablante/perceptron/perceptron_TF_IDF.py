...
print("PERCEPTRON (MLP) OPTIMIZADO + TF-IDF (COMPARATIVA JUSTA)")
...
# Usamos 5000 features para igualar la complejidad del BoW anterior
try:
    vectorizer = joblib.load('models/vec_tfidf_word.joblib')
    print("Vectorizador cargado.")
    X_train_tfidf = vectorizer.transform(X_train).toarray()
except:
    print("Ajustando vectorizador...")
    vectorizer = TfidfVectorizer(max_features=5000)
    X_train_tfidf = vectorizer.fit_transform(X_train).toarray()
    # joblib.dump(vectorizer, 'models/vec_tfidf_word.joblib') # Descomentar para guardar
...
# 4. MODELO "ESPEJO" (ARQUITECTURA COMPARTIDA CON BERT)
class MLPMirrorClassifier(nn.Module):
    def __init__(self, input_dim, num_classes, dropout=0.5):
        ...
        # Activaciones
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout) # REGULARIZACIÓN FUERTE
...
# 5. CONFIGURACIÓN ENTRENAMIENTO (IGUALADA A BERT)
# Pesos de clase para balancear la Loss
...
# 6. TRAINING LOOP
print("\n--- INICIANDO ENTRENAMIENTO COMPARATIVO (TF-IDF) ---")
history = {'loss': [], 'val_f1': []}
best_f1 = 0.0
patience = 10
counter = 0
