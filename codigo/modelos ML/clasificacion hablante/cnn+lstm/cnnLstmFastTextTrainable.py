"""
CNN-LSTM Híbrido con FastText Trainable
CNN extrae features → LSTM captura dependencias → clasificación con embeddings trainables
"""
...
print("CNN-LSTM + FASTTEXT TRAINABLE")
...
ft_model = FastText(sentences=X_train, vector_size=EMBEDDING_DIM, window=5, min_count=2, workers=4, sg=1, epochs=10, min_n=3, max_n=6)
...
model = CNNLSTMFastTextTrainable(vocab_size, EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZE, HIDDEN_DIM, num_classes, DROPOUT, embedding_matrix).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnnlstm_fasttext_trainable.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_cnnlstm_fasttext_trainable.pth'))
model.eval()
...
plt.title('CNN-LSTM + FastText Trainable')
plt.tight_layout()
plt.savefig('confusion_matrix_cnnlstm_fasttext_trainable.png', dpi=300)
print("✓ Completado")
