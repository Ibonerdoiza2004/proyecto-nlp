"""
CNN-LSTM Híbrido con TF-IDF
TF-IDF → Dense embedding layer → CNN → LSTM → clasificación
"""
...
model = CNNLSTMTFIDFClassifier(X_train_tfidf.shape[1], EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZE, HIDDEN_DIM, num_classes, DROPOUT).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnnlstm_tfidf.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_cnnlstm_tfidf.pth'))
model.eval()
...
plt.title('CNN-LSTM + TF-IDF')
plt.tight_layout()
plt.savefig('confusion_matrix_cnnlstm_tfidf.png', dpi=300)
print("✓ Completado")
