"""
CNN-LSTM Híbrido con Bag of Words
BoW → Dense embedding → CNN extrae features → LSTM captura dependencias → clasificación
"""
...
model = CNNLSTMBoWClassifier(X_train_bow.shape[1], EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZE, HIDDEN_DIM, num_classes, DROPOUT).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnnlstm_bow.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_cnnlstm_bow.pth'))
model.eval()
...
plt.title('CNN-LSTM + BoW')
plt.tight_layout()
plt.savefig('confusion_matrix_cnnlstm_bow.png', dpi=300)
print("✓ Completado")
