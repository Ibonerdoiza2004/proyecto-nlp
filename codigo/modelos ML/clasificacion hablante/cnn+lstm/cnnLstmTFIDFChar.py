"""
CNN-LSTM con TF-IDF Character-level
TF-IDF char n-grams → Dense → CNN extrae features → LSTM temporal
"""
...
MAX_FEATURES, EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZE, HIDDEN_DIM = 3000, 128, 64, 3, 64
DROPOUT, BATCH_SIZE, EPOCHS, SEQ_LENGTH = 0.4, 32, 25, 20

print("CNN-LSTM + TF-IDF CHARACTER-LEVEL")
...
class CNNLSTMTFIDFCharClassifier(nn.Module):
    def __init__(self, input_dim, embedding_dim, num_filters, kernel_size, hidden_dim, num_classes, dropout):
        super(CNNLSTMTFIDFCharClassifier, self).__init__()
    ...
    def forward(self, x):
        ...
        embedded = self.embedding_layer(x).view(batch_size, seq_len, -1).transpose(1, 2)
...
model = CNNLSTMTFIDFCharClassifier(X_train_tfidf.shape[1], EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZE, HIDDEN_DIM, num_classes, DROPOUT).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnnlstm_tfidf_char.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_cnnlstm_tfidf_char.pth'))
model.eval()
...
plt.title('CNN-LSTM + TF-IDF Char')
plt.tight_layout()
plt.savefig('confusion_matrix_cnnlstm_tfidf_char.png', dpi=300)
print("✓ Completado")
