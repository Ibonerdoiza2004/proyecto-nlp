"""
LSTM con TF-IDF Character-level
TF-IDF char n-grams (2-5) → Dense embedding layer → LSTM Bidireccional
"""
...
print("LSTM + TF-IDF CHARACTER-LEVEL")
...
# TF-IDF a nivel de caracteres (n-gramas 2-5)
...
print(f"TF-IDF char features: {X_train_tfidf.shape[1]}")

class LSTMTFIDFCharClassifier(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, num_layers, num_classes, dropout):
        super(LSTMTFIDFCharClassifier, self).__init__()
...
model = LSTMTFIDFCharClassifier(X_train_tfidf.shape[1], EMBEDDING_DIM, HIDDEN_DIM, NUM_LAYERS, num_classes, DROPOUT).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_lstm_tfidf_char.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_lstm_tfidf_char.pth'))
model.eval()
...
plt.title('LSTM + TF-IDF Char')
plt.tight_layout()
plt.savefig('confusion_matrix_lstm_tfidf_char.png', dpi=300)
print("✓ Completado")
