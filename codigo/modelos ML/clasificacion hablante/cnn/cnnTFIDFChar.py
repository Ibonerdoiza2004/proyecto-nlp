"""
CNN con TF-IDF Character-level
TF-IDF char n-grams → Dense embedding → CNN múltiples kernels
"""
...
MAX_FEATURES, EMBEDDING_DIM, NUM_FILTERS = 3000, 128, 100
KERNEL_SIZES, DROPOUT, BATCH_SIZE, EPOCHS, SEQ_LENGTH = [2, 3, 4, 5], 0.5, 32, 30, 50

print("CNN + TF-IDF CHARACTER-LEVEL")
...
model = CNNTFIDFCharClassifier(X_train_tfidf.shape[1], EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZES, num_classes, DROPOUT).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnn_tfidf_char.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_cnn_tfidf_char.pth'))
model.eval()
...
plt.title('CNN + TF-IDF Char')
plt.tight_layout()
plt.savefig('confusion_matrix_cnn_tfidf_char.png', dpi=300)
print("✓ Completado")
