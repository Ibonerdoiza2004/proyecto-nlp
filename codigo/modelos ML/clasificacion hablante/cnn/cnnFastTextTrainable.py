"""
CNN con FastText Trainable
FastText embeddings entrenables con char n-grams + CNN múltiples kernels
"""
...
model = CNNFastTextTrainable(vocab_size, EMBEDDING_DIM, NUM_FILTERS, KERNEL_SIZES, num_classes, DROPOUT, embedding_matrix).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnn_fasttext_trainable.pth')
    if epoch % 5 == 0:
        print(f"Epoch {epoch+1}: Val Acc = {val_acc:.4f}")

model.load_state_dict(torch.load('models/best_cnn_fasttext_trainable.pth'))
model.eval()
...
plt.title('CNN + FastText Trainable')
plt.tight_layout()
plt.savefig('confusion_matrix_cnn_fasttext_trainable.png', dpi=300)
print("✓ Completado")
