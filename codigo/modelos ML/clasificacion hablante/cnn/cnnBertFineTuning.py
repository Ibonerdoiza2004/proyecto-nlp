"""
CNN con BERT Fine-tuning Completo
Fine-tuning end-to-end de BERT + CNN para clasificación
"""
...
NUM_FILTERS, KERNEL_SIZES, DROPOUT = 100, [2, 3, 4], 0.5
BATCH_SIZE, EPOCHS, MAX_LEN = 8, 10, 128
BERT_LR, CNN_LR = 2e-5, 1e-4

print("CNN + BERT FINE-TUNING")
...
class CNNBertFineTuning(nn.Module):
    def __init__(self, bert_model_name, num_filters, kernel_sizes, num_classes, dropout):
        super(CNNBertFineTuning, self).__init__()
        self.bert = AutoModel.from_pretrained(bert_model_name)
        bert_dim = self.bert.config.hidden_size
        self.convs = nn.ModuleList([nn.Conv1d(bert_dim, num_filters, k) for k in kernel_sizes])
    ...
    def forward(self, input_ids, attention_mask):
        ...
        for conv, bn in zip(self.convs, self.batch_norms):
            conv_out = self.relu(bn(conv(sequence_output)))
...
model = CNNBertFineTuning(BERT_MODEL, NUM_FILTERS, KERNEL_SIZES, num_classes, DROPOUT).to(device)

bert_params = list(model.bert.parameters())
cnn_params = [p for n, p in model.named_parameters() if 'bert' not in n]
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnn_bert_finetuning.pth')

model.load_state_dict(torch.load('models/best_cnn_bert_finetuning.pth'))
model.eval()
...
plt.title('CNN + BERT Fine-tuning')
plt.tight_layout()
plt.savefig('confusion_matrix_cnn_bert_finetuning.png', dpi=300)
print("✓ Completado")
