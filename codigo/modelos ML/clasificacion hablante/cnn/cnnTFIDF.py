"""
Clasificación de Hablantes usando CNN con TF-IDF
Arquitectura: TF-IDF → Dense (embedding denso) → CNN (múltiples kernels) → Dense
Técnicas: TF-IDF vectorization, Multiple kernel sizes, Batch normalization, Max pooling, Gradient Clipping
Fuentes: PDF págs 15-18 (TF-IDF), págs 25-30 (CNNs)
"""
...
WEIGHT_DECAY = 1e-5
GRAD_CLIP = 5.0
SEQ_LENGTH = 50  # Longitud de secuencia para CNN
...
print(f"\nShape train secuencial: {X_train_seq.shape}")
...
# Modelo CNN con TF-IDF
class CNNTFIDFClassifier(nn.Module):
    def __init__(self, input_dim, embedding_dim, num_filters, kernel_sizes, num_classes, dropout):
        super(CNNTFIDFClassifier, self).__init__()
        
        # Capa densa para embedding
    ...
    def forward(self, x):
        ...
        # Concatenar features de todos los kernels
        concatenated = torch.cat(conv_outputs, dim=1)  # [batch, num_filters * len(kernels)]
...
# Crear modelo
print("\n" + "="*60)
print("CONSTRUYENDO MODELO CNN + TF-IDF")
print("="*60)

model = CNNTFIDFClassifier(
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'models/best_cnn_tfidf.pth')
        print(f"✓ Mejor modelo guardado (val_acc: {val_acc:.4f})")

# Cargar mejor modelo
model.load_state_dict(torch.load('models/best_cnn_tfidf.pth'))
...
plt.title('Matriz de Confusión - CNN + TF-IDF')
plt.ylabel('Real')
plt.xlabel('Predicción')
plt.tight_layout()
plt.savefig('confusion_matrix_cnn_tfidf.png', dpi=300, bbox_inches='tight')
...
axes[0].set_title('Accuracy - CNN + TF-IDF')
...
plt.tight_layout()
plt.savefig('training_history_cnn_tfidf.png', dpi=300, bbox_inches='tight')
